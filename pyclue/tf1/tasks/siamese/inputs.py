...
class SiameseProcessorBase(object):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            if isinstance(line, str):
                line = [self.labels[0], line, line]
            elif isinstance(line, list) and len(line) == 2:
                line = [self.labels[0], line[0], line[1]]
...
class SiameseProcessorFile(SiameseProcessorBase):
...
class SiameseInputFnBase(object):
...
class SiameseInputFnFile(SiameseInputFnBase):
    """Data converters for sequence sentence_pair data sets saved as tf-record format."""
